...
function DivisionColumn({
  ...
  return (
    <div className="flex flex-col overflow-hidden rounded-md shadow-sm">
      ...
      <div className={cn('px-3 py-2 text-center', style.bg, style.text)}>
        <p className="text-[11px] font-semibold uppercase leading-tight tracking-wide">
          {node.label}
        </p>
        {node.positionTitle ? (
          <div className="mt-2 flex items-center justify-between gap-2 rounded bg-black/20 px-2 py-1 text-xs font-medium">
  ...
  return (
    <div className="space-y-1">
      <Link
        ...
        className="block text-[11px] font-semibold leading-snug hover:underline"
      >
        {node.label}
      </Link>
      {node.positionTitle ? (
